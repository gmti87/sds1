...
class iOSBuilder {
  ...
  async prepareJobAsync(archiveUrl) {
    if (this.ctx.buildProfile.workflow === _easJson().Workflow.Generic) {
      return (0, _buildTools().sanitizeJob)((await this.prepareGenericJobAsync(archiveUrl, this.ctx.buildProfile)));
    } else if (this.ctx.buildProfile.workflow === _easJson().Workflow.Managed) {
      return (0, _buildTools().sanitizeJob)((await this.prepareManagedJobAsync(archiveUrl, this.ctx.buildProfile)));
    } else {
  ...
  async ensureCredentialsAsync() {
    ...
    } catch (_unused) {// credentials.json can not exist and that is fine.
  ...
  async configureProjectAsync() {
    ...
    const spinner = (0, _ora().default)('Configuring the Xcode project');
    const bundleIdentifier = await (0, _ios().getBundleIdentifier)(this.ctx.commandCtx.projectDir, this.ctx.commandCtx.exp);
